...
const ReportDetailsNHC = () => {
  ...
  const goToUpdate = () => {
    navigate(`/main/update-nhc/${reportId}`); // Navigate to the UpdateNHC component
  };
  ...
  const exportToPrint = (report) => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>NHC Report - ${report.name}</title>
          <style>
            body {
  ...
  return (
    <div className="rnhc-container">
      ...
      <h2 className="rnhc-title">NHC OF {report.name}</h2>
...
export default ReportDetailsNHC;